...
export function AddFeedingDialog({
  ...
  async function submit(volumeMl: number) {
    ...
    setPending(true);
    const isoFedAt = showTime
      ? localInputToISO(fedAt)
      : new Date().toISOString();
    const res = await addFeeding({ volumeMl, fedAt: isoFedAt });
    setPending(false);
    if (!res.ok) {
      toast.error(
        res.error === "INVALID_VOLUME"
          ? "Dung tích không hợp lệ"
          : "Lưu thất bại",
      );
      return;
    }
  ...
  return (
    ...
    >
      ...
      <DialogContent>
        ...
        <DialogClose asChild>
          <button type="button" title="Đóng" className="hidden" aria-hidden />
        </DialogClose>
      </DialogContent>
    </Dialog>
  );
